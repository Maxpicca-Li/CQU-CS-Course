...
class DataBase:
    ...
    def create(self,t_name, cols, types, pk:str):
        ...
        if tb_exists:
            ...
            # 索引文件
            index = {}     #先初始化为0
            self.__save_index(t_name, index)
            # 字段文件
            field = {}
    ...
    def insert(self,t_name:str, row:tuple, cols:tuple=None):
        index = self.__load_index(t_name)
        ...
        # 检查有无重复
        if metadata[self.curr_pk] in index:
        ...
        df.loc[lid] = metadata
        index[pk_value] = BLOCKSIZE*bid+lid    #在索引里记录数据的位置
        # save
    ...
    def delete(self,t_name,pk_value):
        ...
        block = self.__load_block(t_name)
        # 需要判断索引是否存在
        if pk_value in index.keys():
            print(f"删除主键为{pk_value}的数据")
            #删除索引
            num = index[pk_value]
            bid = num//BLOCKSIZE
            lid = num%BLOCKSIZE
            if block[bid]&(1<<lid)==0:
    ...
    def __load_index(self,t_name) -> dict: 
        path = os.path.join(DATABASE,self.curr_db,t_name,INDEX)
    ...
    def __save_index(self,t_name,index:dict):
        path = os.path.join(DATABASE,self.curr_db,t_name,INDEX)
